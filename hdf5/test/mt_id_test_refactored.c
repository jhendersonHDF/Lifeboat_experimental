...
#include <stdatomic.h> /* TODO: eventually this should just be included by h5test.h */
...
static void serial_test_2(void);
static void serial_test_3(void);
static void serial_test_4(void);

static void * mt_test_fcn_1(void * params);
static void * mt_test_fcn_2(void * params);

static void mt_test_fcn_1_serial_test(void);
static void mt_test_1(void);
static void mt_test_1_helper(int num_threads);

static void mt_test_2(void);
...
serial_test_2(void)
{
    const mt_test_params_t *test_params = (const mt_test_params_t *)GetTestParameters();
    hbool_t cs = FALSE;
    hbool_t ds = FALSE;
    hbool_t rpt_failures = TRUE;
    int err_cnt = 0;
...
main(int argc, char **argv)
{
    ...
    /* SetTestVerbosity(VERBO_NONE); XXX */
...
#ifdef H5_HAVE_MULTITHREAD
    /* Add tests */
    AddTest("serial_test_1", serial_test_1, NULL, "smoke check test for various H5I operations on IDs", NULL);
    ...
    AddTest("serial_test_2", serial_test_2, NULL, "smoke check test for H5I ID registrations", &test_params);

    AddTest("serial_test_3", serial_test_3, NULL, "another smoke check test for H5I ID registrations", NULL);
    AddTest("serial_test_4", serial_test_4, NULL, "smoke check test for H5I future ID functionality", NULL);
    AddTest("mt_test_fcn_1_serial_test", mt_test_fcn_1_serial_test, NULL, "serial smoke check test for multi-thread helper function", NULL);
    AddTest("mt_test_1", mt_test_1, NULL, "multi-thread H5I smoke check test", NULL);
    AddTest("mt_test_2", mt_test_2, NULL, "multi-thread H5I smoke check test", NULL);

    /* Parse command line arguments */
    TestParseCmdLine(argc, argv);

    /* Initialize globals used by the multithread tests */
    if (init_globals() < 0) {
        IncTestNumErrs();
        fprintf(stderr, "failed to initialize globals used by tests\n");
        goto exit;
    }
...
exit:
    ...
    exit(GetTestNumErrs() > 0 ? EXIT_FAILURE : EXIT_SUCCESS);
}
